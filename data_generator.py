...
def put_data(conn:str, dbms:str, table:str, row:str)->bool:
    """
    Execute PUT command
    :args:
...
def main():
    """
    Store traffic data over the month of April from 2 locations into operator node(s)
    :positional arguments:
      conn                  comma separated REST connection information
    :optional arguments:
      -h, --help                show this help message and exit
      --db-name     DB_NAME     logical database name
      --table-name  TABLE_NAME  logical table name. If None each data set will have its own table (by default)
    :params:
    ...
    parser.add_argument('--table-name', type=str, default=None, help='logical table name. If None each data set will have its own table (by default)')
        ...
        for location in DATA:
            for row in DATA[location]:
                if args.table_name is None:
                    put_data(conn=conn, dbms=args.dbms, table=location, row=json.dumps(row))
                if args.table_name is None:
                    put_data(conn=conn, dbms=args.dbms, table=args.table_name, row=json.dumps(row))
    else:
        for conn in conns_list:
            location = list(DATA.keys())[-1]
            if conns_list.index(conn) % 2 == 0:
